...
class EmailJob(db.Model):
    ...
    def get_export_data(cls):
        jobs = cls.query.with_entities(
            cls.subject, cls.created, cls.recipient_count,
            cls.text_body, cls.html_body,
        )
        data = {
            'public': {
                'jobs': jobs,
            },
            'tables': ['email_job', 'email_recipient'],
        }
...
EmailJob.recipient_count = column_property(select([func.count(EmailJobRecipient.job_id)]).where(
