...
def line_up_proposal(proposal_id, slug=None):
    ...
    if (request.method == "POST") and not current_user.is_anonymous:
        ...
        flash(msg)
        return redirect(
            url_for(".line_up_proposal", proposal_id=proposal.id, slug=proposal.slug)
        )
    ...
    return render_template(
        "schedule/line-up-proposal.html",
        proposal=proposal,
        is_fave=is_fave,
...
def line_up_external(event_id, slug=None):
    ...
    if (request.method == "POST") and not current_user.is_anonymous:
        ...
        flash(msg)
        return redirect(
            url_for(".line_up_external", event_id=event.id, slug=event.slug)
        )
    ...
    return render_template(
        "schedule/line-up-external.html",
        event=event,
        is_fave=is_fave,
