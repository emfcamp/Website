...
def get_volunteer_sort_dict(parameters):
    sort_keys = {
        'name': lambda v: (v.nickname, v.volunteer_email)
    }
...
def filter_volunteer_request():
    ...
    if desired_roles:
        filtered = True
        volunteers = Volunteer.query.filter(Volunteer.interested_roles.any(Role.name.in_(desired_roles))).all()
    else:
...
def emailvolunteers():
    ...
    return render_template('notification/email_volunteers.html', volunteers=volunteers,
                           new_qs=non_sort_query_string, roles=ordered_roles,
                           filtered=filtered, total_volunteers=Volunteer.query.count())
...
def message_batch():
    ...
    return render_template('notification/message_batch.html',
                           form=form, volunteers=volunteers)
...
def notify_email(volunteer, subject, message):
    ...
    while True:
        ...
        try:
            ...
            app.logger.error('Failed to email volunteer %s, ABORTING: %s', volunteer.volunteer_email, e)
            return False
