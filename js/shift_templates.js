...
var newRowCount = document.querySelectorAll('#shift-templates-body [data-is-new]').length;
...
function addNewRow(sourceFieldset) {
    ...
    if (sourceFieldset) {
        var srcInputs = sourceFieldset.querySelectorAll('input:not(.delete-row-checkbox):not([type=hidden]), select, textarea');
        var dstInputs = fieldset.querySelectorAll('input:not(.delete-row-checkbox):not([type=hidden]), select, textarea');
